...
```
feat(mes): add work order CRUD API

Implement create, read, update, delete operations for work order.
Includes input validation and permission checks.
...
3. **范围可选**：`feat(mes)` 或 `fix(mes-page)` 有助于定位变更模块。
4. **关联 Issue**：在正文中使用 `Closes #123` 或 `Fixes MES-42` 自动关联。
...
**示例：**
- `feat(mes): add work order module`
...
### ✨ 新功能
- feat(mes): add work order CRUD API (MES-42)
- feat(mes-page): add production dashboard (MES-43)

### 🐛 Bug 修复
- fix(mes): resolve NPE on empty order list (MES-55)
- fix(mes-page): date picker not initializing correctly (MES-57)

### 🔧 重构
- refactor(mes): extract order validation logic to service layer

### 📝 文档
- docs: update API documentation for work order endpoints
...
Multica Issue (MES-42)
    ↓  创建 feature 分支
GitHub: feature/mes-add-work-order-module
    ↓  提交代码，Commit 中引用 Multica Issue ID
Commit: "feat(mes): add work order CRUD API"
...
**命名关联规则：**
- feature 分支：`feature/<multica-issue-key>-<描述>`，如 `feature/mes-42-work-order-crud`
...
```bash
# 创建 PR 后
multica issue metadata set MES-42 --key pr_url --value https://github.com/huangxn29/mes/pull/42
...
```bash
git add -A
git commit -m "feat(mes): add work order CRUD API"
git push -u origin feature/mes-add-work-order-module
...
multica repo checkout https://github.com/huangxn29/mes.git

# 基于 develop 创建 feature 分支
git checkout develop
git pull
git checkout -b feature/mes-42-work-order-crud
```
...
# 推送
git push -u origin feature/mes-42-work-order-crud
```
...
multica issue comment add MES-42 --content "PR created: https://github.com/huangxn29/mes/pull/42"
```
...
# 更新元数据（如有部署 URL）
multica issue metadata set MES-42 --key deploy_url --value https://example.com

# 清理过期元数据
multica issue metadata delete MES-42 --key pr_url
```
  ...
  build:
    ...
    steps:
      ...
      - name: Build & Test
        run: |
          # 根据项目语言调整
          # mvn clean verify 或 npm run build && npm test
      - name: Lint Check
        run: |
          # npm run lint 或 mvn checkstyle:check
...
git checkout develop
git pull
git checkout -b feature/mes-add-work-order-module
...
# 提交
git add .
git commit -m "feat(mes): add work order list API"
